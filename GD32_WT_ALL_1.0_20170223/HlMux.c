...
#ifdef TTL_FOR_NOT_START_EMSD
#define RIL_DUMMY0_LINK_FILE     "/dev/dummyPts0"
#define RIL_DUMMY1_LINK_FILE     "/dev/dummyPts1"
#define RIL_DUMMY2_LINK_FILE     "/dev/dummyPts2"
#define RIL_DUMMY3_LINK_FILE     "/dev/dummyPts3"
#define RIL_DUMMY4_LINK_FILE     "/dev/dummyPts4"
#define RIL_DUMMY5_LINK_FILE     "/dev/dummyPts5"
#define RIL_DUMMY6_LINK_FILE     "/dev/dummyPts6"
#endif

static int RcFd_Master = 0;
static int RilFd_Master = 0; 
static int PppFd_Master = 0; 
static int UartFd = 0; 
#ifdef TTL_FOR_NOT_START_EMSD
static int RilFd_Dummy0_Master = 0; 
static int RilFd_Dummy1_Master = 0; 
static int RilFd_Dummy2_Master = 0; 
static int RilFd_Dummy3_Master = 0; 
static int RilFd_Dummy4_Master = 0; 
static int RilFd_Dummy5_Master = 0; 
static int RilFd_Dummy6_Master = 0; 
#endif
...
static void UartMuxDataReq(char *pUartSndBuff, unsigned short usUartLen)
{
    ...
    while(usUartLen > 0)
    {
        SendLen = write(UartFd, pUartSndBuff + SendOffset, usUartLen);
        SendOffset  += SendLen;
        usUartLen -= SendLen;
        if(0 == SendLen)
        {
...
static unsigned char GetFrameFromDataStream()
{
    ...
    /* FrameBuff.pFrameData 中存放了一个正常frame帧的数据 */
    return (MUX_TRUE);
}
...
static void UihFrameCreate(char *pBuffer, unsigned short usBufferLen)//zdh    xiangxia fasong zhengeshi shengcheng
{
...
static void ReadRilLoop()
{
    ...
    while(1)
    {
        nRead = read(RilFd_Master, RilReadBuf, MAX_RIL_MSG_LEN);  //zdh   wenjianduqu
        if(nRead > 0)
        {
            MuxTrace(MUX_LOG_LEVEL_2, "nRead = %d", nRead);
            RilReadBuf[nRead] = 0;
...
int main(int argc,char *argv[])
{
    ...
    char*       pPppSlaveName = NULL;
#ifdef TTL_FOR_NOT_START_EMSD
    char*       pDummyPts0SlaveName = NULL;
    char*       pDummyPts1SlaveName = NULL;
    char*       pDummyPts2SlaveName = NULL;
    char*       pDummyPts3SlaveName = NULL;
    char*       pDummyPts4SlaveName = NULL;
    char*       pDummyPts5SlaveName = NULL;
    char*       pDummyPts6SlaveName = NULL;
#endif
...
#ifdef TTL_FOR_NOT_START_EMSD
    system("rm /dev/dummyPts0");
    system("rm /dev/dummyPts1");
    system("rm /dev/dummyPts2");
    system("rm /dev/dummyPts3");
    system("rm /dev/dummyPts4");
    system("rm /dev/dummyPts5");
    system("rm /dev/dummyPts6");
#endif
    ...
    if(0 > (RilFd_Master = open(PTMASTER, O_RDWR)))
    {
        MuxErrorLog("open /dev/ptmx error");
        perror("open /dev/ptmx error");
        exit(EXIT_FAILURE);
...
#ifdef TTL_FOR_NOT_START_EMSD
    //Creat pseudo-terminal for dummyPts0
    if(0 > (RilFd_Dummy0_Master = open(PTMASTER, O_RDWR)))
    {
        MuxErrorLog("open /dev/ptmx error");
        perror("open /dev/ptmx error");
        exit(EXIT_FAILURE);
    }
    MuxTrace(MUX_LOG_LEVEL_2, "open /dev/ptmx return ID = %d", RilFd_Dummy0_Master);
    
    tcgetattr(RilFd_Dummy0_Master, &options); //get the parameters
    options.c_lflag &= ~(ICANON | ECHO | ECHOE | ISIG); //set raw input
    options.c_iflag &= ~(INLCR | ICRNL | IGNCR);
    options.c_oflag &= ~(OPOST| OLCUC| ONLRET| ONOCR| OCRNL); //set raw output
    tcsetattr(RilFd_Dummy0_Master, TCSANOW, &options); //set the parameters
        
    grantpt(RilFd_Dummy0_Master);  /* change permission of slave */
    unlockpt(RilFd_Dummy0_Master); /* unlock slave */
    
    pDummyPts0SlaveName = ptsname(RilFd_Dummy0_Master); //get slave name
    if (pDummyPts0SlaveName == NULL) 
    {
        MuxTrace(MUX_LOG_LEVEL_1, "Get pts name failed");
        exit (EXIT_FAILURE);
    }
    MuxTrace(MUX_LOG_LEVEL_2, "Slave name for dummyPts0 is %s", pDummyPts0SlaveName);
    
    if(symlink(pDummyPts0SlaveName, RIL_DUMMY0_LINK_FILE) < 0)
    {
        MuxErrorLog("symlink error");
        perror("symlink error");
        exit(EXIT_FAILURE);
    ...
    //Creat pseudo-terminal for dummyPts1
    if(0 > (RilFd_Dummy1_Master = open(PTMASTER, O_RDWR)))
    {
        MuxErrorLog("open /dev/ptmx error");
        perror("open /dev/ptmx error");
        exit(EXIT_FAILURE);
    }
    MuxTrace(MUX_LOG_LEVEL_2, "open /dev/ptmx return ID = %d", RilFd_Dummy1_Master);
    
    tcgetattr(RilFd_Dummy1_Master, &options); //get the parameters
    options.c_lflag &= ~(ICANON | ECHO | ECHOE | ISIG); //set raw input
    options.c_iflag &= ~(INLCR | ICRNL | IGNCR);
    options.c_oflag &= ~(OPOST| OLCUC| ONLRET| ONOCR| OCRNL); //set raw output
    tcsetattr(RilFd_Dummy1_Master, TCSANOW, &options); //set the parameters
        
    grantpt(RilFd_Dummy1_Master);  /* change permission of slave */
    unlockpt(RilFd_Dummy1_Master); /* unlock slave */
    
    pDummyPts1SlaveName = ptsname(RilFd_Dummy1_Master); //get slave name
    if (pDummyPts1SlaveName == NULL) 
    {
        MuxTrace(MUX_LOG_LEVEL_1, "Get pts name failed");
        exit (EXIT_FAILURE);
    }
    MuxTrace(MUX_LOG_LEVEL_2, "Slave name for dummyPts1 is %s", pDummyPts1SlaveName);
    
    if(symlink(pDummyPts1SlaveName, RIL_DUMMY1_LINK_FILE) < 0)
    {
        MuxErrorLog("symlink error");
        perror("symlink error");
        exit(EXIT_FAILURE);
    ...
    //Creat pseudo-terminal for dummyPts2
    if(0 > (RilFd_Dummy2_Master = open(PTMASTER, O_RDWR)))
    {
        MuxErrorLog("open /dev/ptmx error");
        perror("open /dev/ptmx error");
        exit(EXIT_FAILURE);
    }
    MuxTrace(MUX_LOG_LEVEL_2, "open /dev/ptmx return ID = %d", RilFd_Dummy2_Master);

    tcgetattr(RilFd_Dummy2_Master, &options); //get the parameters
    options.c_lflag &= ~(ICANON | ECHO | ECHOE | ISIG); //set raw input
    options.c_iflag &= ~(INLCR | ICRNL | IGNCR);
    options.c_oflag &= ~(OPOST| OLCUC| ONLRET| ONOCR| OCRNL); //set raw output
    tcsetattr(RilFd_Dummy2_Master, TCSANOW, &options); //set the parameters
        
    grantpt(RilFd_Dummy2_Master);  /* change permission of slave */
    unlockpt(RilFd_Dummy2_Master); /* unlock slave */

    pDummyPts2SlaveName = ptsname(RilFd_Dummy2_Master); //get slave name
    if (pDummyPts2SlaveName == NULL) 
    {
        MuxTrace(MUX_LOG_LEVEL_1, "Get pts name failed");
        exit (EXIT_FAILURE);
    }
    MuxTrace(MUX_LOG_LEVEL_2, "Slave name for dummyPts2 is %s", pDummyPts2SlaveName);

    if(symlink(pDummyPts2SlaveName, RIL_DUMMY2_LINK_FILE) < 0)
    {
        MuxErrorLog("symlink error");
        perror("symlink error");
        exit(EXIT_FAILURE);
    ...
    //Creat pseudo-terminal for dummyPts3
    if(0 > (RilFd_Dummy3_Master = open(PTMASTER, O_RDWR)))
    {
        MuxErrorLog("open /dev/ptmx error");
        perror("open /dev/ptmx error");
        exit(EXIT_FAILURE);
    }
    MuxTrace(MUX_LOG_LEVEL_2, "open /dev/ptmx return ID = %d", RilFd_Dummy3_Master);

    tcgetattr(RilFd_Dummy3_Master, &options); //get the parameters
    options.c_lflag &= ~(ICANON | ECHO | ECHOE | ISIG); //set raw input
    options.c_iflag &= ~(INLCR | ICRNL | IGNCR);
    options.c_oflag &= ~(OPOST| OLCUC| ONLRET| ONOCR| OCRNL); //set raw output
    tcsetattr(RilFd_Dummy3_Master, TCSANOW, &options); //set the parameters
        
    grantpt(RilFd_Dummy3_Master);  /* change permission of slave */
    unlockpt(RilFd_Dummy3_Master); /* unlock slave */

    pDummyPts3SlaveName = ptsname(RilFd_Dummy3_Master); //get slave name
    if (pDummyPts3SlaveName == NULL) 
    {
        MuxTrace(MUX_LOG_LEVEL_1, "Get pts name failed");
        exit (EXIT_FAILURE);
    }
    MuxTrace(MUX_LOG_LEVEL_2, "Slave name for dummyPts3 is %s", pDummyPts3SlaveName);

    if(symlink(pDummyPts3SlaveName, RIL_DUMMY3_LINK_FILE) < 0)
    {
        MuxErrorLog("symlink error");
        perror("symlink error");
        exit(EXIT_FAILURE);
    ...
    //Creat pseudo-terminal for dummyPts4
    if(0 > (RilFd_Dummy4_Master = open(PTMASTER, O_RDWR)))
    {
        MuxErrorLog("open /dev/ptmx error");
        perror("open /dev/ptmx error");
        exit(EXIT_FAILURE);
    }
    MuxTrace(MUX_LOG_LEVEL_2, "open /dev/ptmx return ID = %d", RilFd_Dummy4_Master);

    tcgetattr(RilFd_Dummy4_Master, &options); //get the parameters
    options.c_lflag &= ~(ICANON | ECHO | ECHOE | ISIG); //set raw input
    options.c_iflag &= ~(INLCR | ICRNL | IGNCR);
    options.c_oflag &= ~(OPOST| OLCUC| ONLRET| ONOCR| OCRNL); //set raw output
    tcsetattr(RilFd_Dummy4_Master, TCSANOW, &options); //set the parameters
        
    grantpt(RilFd_Dummy4_Master);  /* change permission of slave */
    unlockpt(RilFd_Dummy4_Master); /* unlock slave */

    pDummyPts4SlaveName = ptsname(RilFd_Dummy4_Master); //get slave name
    if (pDummyPts4SlaveName == NULL) 
    {
        MuxTrace(MUX_LOG_LEVEL_1, "Get pts name failed");
        exit (EXIT_FAILURE);
    }
    MuxTrace(MUX_LOG_LEVEL_2, "Slave name for dummyPts4 is %s", pDummyPts4SlaveName);

    if(symlink(pDummyPts4SlaveName, RIL_DUMMY4_LINK_FILE) < 0)
    {
        MuxErrorLog("symlink error");
        perror("symlink error");
        exit(EXIT_FAILURE);
    ...
    //Creat pseudo-terminal for dummyPts5
    if(0 > (RilFd_Dummy5_Master = open(PTMASTER, O_RDWR)))
    {
        MuxErrorLog("open /dev/ptmx error");
        perror("open /dev/ptmx error");
        exit(EXIT_FAILURE);
    }
    MuxTrace(MUX_LOG_LEVEL_2, "open /dev/ptmx return ID = %d", RilFd_Dummy5_Master);

    tcgetattr(RilFd_Dummy5_Master, &options); //get the parameters
    options.c_lflag &= ~(ICANON | ECHO | ECHOE | ISIG); //set raw input
    options.c_iflag &= ~(INLCR | ICRNL | IGNCR);
    options.c_oflag &= ~(OPOST| OLCUC| ONLRET| ONOCR| OCRNL); //set raw output
    tcsetattr(RilFd_Dummy5_Master, TCSANOW, &options); //set the parameters
        
    grantpt(RilFd_Dummy5_Master);  /* change permission of slave */
    unlockpt(RilFd_Dummy5_Master); /* unlock slave */

    pDummyPts5SlaveName = ptsname(RilFd_Dummy5_Master); //get slave name
    if (pDummyPts5SlaveName == NULL) 
    {
        MuxTrace(MUX_LOG_LEVEL_1, "Get pts name failed");
        exit (EXIT_FAILURE);
    }
    MuxTrace(MUX_LOG_LEVEL_2, "Slave name for dummyPts5 is %s", pDummyPts5SlaveName);

    if(symlink(pDummyPts5SlaveName, RIL_DUMMY5_LINK_FILE) < 0)
    {
        MuxErrorLog("symlink error");
        perror("symlink error");
        exit(EXIT_FAILURE);
    ...
    //Creat pseudo-terminal for dummyPts6
    if(0 > (RilFd_Dummy6_Master = open(PTMASTER, O_RDWR)))
    {
        MuxErrorLog("open /dev/ptmx error");
        perror("open /dev/ptmx error");
        exit(EXIT_FAILURE);
    }
    MuxTrace(MUX_LOG_LEVEL_2, "open /dev/ptmx return ID = %d", RilFd_Dummy6_Master);

    tcgetattr(RilFd_Dummy6_Master, &options); //get the parameters
    options.c_lflag &= ~(ICANON | ECHO | ECHOE | ISIG); //set raw input
    options.c_iflag &= ~(INLCR | ICRNL | IGNCR);
    options.c_oflag &= ~(OPOST| OLCUC| ONLRET| ONOCR| OCRNL); //set raw output
    tcsetattr(RilFd_Dummy6_Master, TCSANOW, &options); //set the parameters
        
    grantpt(RilFd_Dummy6_Master);  /* change permission of slave */
    unlockpt(RilFd_Dummy6_Master); /* unlock slave */

    pDummyPts6SlaveName = ptsname(RilFd_Dummy6_Master); //get slave name
    if (pDummyPts6SlaveName == NULL) 
    {
        MuxTrace(MUX_LOG_LEVEL_1, "Get pts name failed");
        exit (EXIT_FAILURE);
    }
    MuxTrace(MUX_LOG_LEVEL_2, "Slave name for dummyPts6 is %s", pDummyPts6SlaveName);

    if(symlink(pDummyPts6SlaveName, RIL_DUMMY6_LINK_FILE) < 0)
    {
        MuxErrorLog("symlink error");
        perror("symlink error");
        exit(EXIT_FAILURE);
